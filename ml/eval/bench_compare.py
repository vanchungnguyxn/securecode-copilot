...
def main():
    ...
    for key in ("detector_test", "multilingual_pairs"):
        ...
        md.append("")
    md.append("## CodeT5 fix soft-match (LEGACY — do not use as primary claim)")
    md.append("")
    md.append(
        "Soft-match on `sft_pairs` overlaps the training family and is easy to satisfy "
        "(prefix / token overlap). For thesis-grade fix quality use held-out evaluation:"
    )
    md.append("")
    md.append("- Protocol: [`ml/eval/FIX_EVAL.md`](../FIX_EVAL.md)")
    md.append("- Harness: `python ml/eval/eval_fix_heldout.py --provider local`")
    md.append("- Metrics: Exact · CodeBLEU · Compile · Unit · Security · Functional · Human rubric")
    md.append("")
    ...
    md.append("- **Fix quality**: report held-out unit/security rates, not this soft-match block alone.")
    md.append("- **Multilingual detector**: see [`bench_multilingual.md`](bench_multilingual.md) (per-language tables; prefer `cvefixes_pairs` tier).")
